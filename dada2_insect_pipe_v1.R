...
filtfile<-"eDNA_db/Atacama/Atacama_euka02_C6RNK"
...
#
#fastqfile<-"eDNA_db/Red_sea_surface/RS_euka02_BTKGR/RS_euka02_cut"
...
#
#fastqfile<-"eDNA_db/Atacama/Atacama_CO1_C6RMT/Atacama_CO1_cut"
...
dada_ref<-"SILVA_132_trimmed_euka02_dada2_names.fasta"
#   dada_ref<-"SILVA_132_trimmed_euka02_dada2_names_with_ortega_macroalgae.fasta"
#  dada_ref<-"SILVA_132_trimmed_euka02_dada2_names_with_ortega_bachmann_macroalgae.fasta"
...
dada_ref<-"MIDORI_LONGEST_20180221_COI_trimmed_dada.fasta"
}
# rbclmini
if (primer_name=="rbclmini") {
  ...
  dada_ref<-"ncbi_rbcl_trimmed_rbclmini_dada2_names.fasta"
  }
# 18smini
if (primer_name=="18smini") {
  ...
  dada_ref<-"SILVA_132_trimmed_18smini_dada2_names.fasta"
  }
# vert
if (primer_name=="vert") {
  ...
  dada_ref<-"ncbi_12s_euk_only_dada2_virtualPCR.fasta"
...
if (primer_name=="18s_stoeck") {
  ...
  dada_ref<-"SILVA_132_trimmed_18s_stoeck_dada2_names.fasta"
  }
...
filter_folder<-paste("filtered",study_name,primer_name,sep="__")  #
...
sample.names <- sapply(strsplit(basename(fnFs), "_"), `[`, 1)  # clean up miseq double names, use later on
## plot errors   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
###  dereplicate #####################################
## need to coreect length if one smaple does not have sequences
...
####
####
####      Clear environment (need to rerun 1-130) or   Close and re-open R      ####################
###
#####
################################################################################################################
################################################################################################################
...
# out_name<-"RS_surface__euka02_with_ortega"
